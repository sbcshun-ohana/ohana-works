...
> ## 【設計変更 v1.1(2026-08-28・俊決定)】請求単位を「世帯1本」→「園児個別」へ変更
> - 保護者は**園児ごとに請求を受け、園児ごとに支払う**。入金確認も園児単位。兄弟がいる家庭は園児の数だけ請求が届く(俊了承)。
> - 理由: 世帯確定の移行作業が請求開始の前提になると保育園側の準備が煩わしいため。
> - 本書の §1(世帯・代表保護者)と §7〜8 の「請求先=household」の記述は
>   **「請求先=園児(閲覧・支払可能者=guardian_child_links の登録保護者・AC-19は"自分の子のみ"に読み替え)」**として実装する。
> - households 自体は**重要事項説明書の世帯同意(310・一人の同意=家族全員分)と入園手続き(218)で存続**(変更なし・俊確認済み)。
>   文書同意の世帯まとめは今後も継続する。
> - 世帯割当RPC(384/392)は保留(請求の前提から除外・stagingに適用済みのため残置)。世帯管理画面は撤去済み。
> - Phase1〜3の成果(料金マスター386-388・園児契約389-391・CSVレイアウト)は**すべて園児単位のため影響なし**。
