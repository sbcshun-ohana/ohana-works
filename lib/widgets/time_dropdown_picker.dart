...
/// 保育業務(childcare)の時刻入力を、Material標準の時計ダイアログ(showTimePicker)ではなく
/// 「時」「分」のプルダウン選択に統一するための共通ピッカー(俊確定 2026-08-12)。
///
/// showTimePicker と同じ呼び出し形(context / initialTime)・同じ戻り値(`Future<TimeOfDay?>`)の
/// ドロップイン置換。既存の「タップ→ダイアログ→TimeOfDayを返す」フローをそのまま保てるよう
/// ダイアログ形式にしている。分は既定で1分刻み(showTimePicker と同じ精度)。
