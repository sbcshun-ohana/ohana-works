-- 保護者アプリ Phase A: 招待発行・招待受諾・アカウント状態変更のRPC
-- QRトークンの発行・消費はissue-qr-token/resolve-qr-punchと同じ理由でEdge Function側
-- (service role・秘密鍵管理)に実装する。本ファイルでは扱わない。

-- 職員(施設管理者)による招待発行(初回招待に相当)。
...
-- 主たる保護者による2人目の主たる保護者・追加保護者の招待(変更5)。
...
-- 招待受諾: 呼び出し前にSupabase Auth側でサインアップ/ログイン済みであることが前提
-- (auth.uid()がnullの場合は拒否)。既に別の園児向けguardians行を持つ場合は
-- 新規作成せず、guardian_child_linksの追加のみ行う。
...
-- アカウント停止・再開。実際のセッション無効化(強制ログアウト)はAdmin Auth APIが必要なため
-- Edge Function側で別途実装する。本RPCは状態変更と操作履歴の記録のみ行う。
create or replace function suspend_guardian_account(p_guardian_id uuid, p_reason text)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if not exists (
    ...
    where gcl.guardian_id = p_guardian_id and staff_manages_guardian_data(gcl.child_id)
  ) then
    raise exception 'not authorized';
  end if;

  update guardians set status = 'suspended' where id = p_guardian_id;
  insert into guardian_account_actions (guardian_id, action, performed_by, reason)
  values (p_guardian_id, 'suspended', my_employee_id(), p_reason);
end;
$$;

create or replace function reactivate_guardian_account(p_guardian_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update guardians set status = 'active' where id = p_guardian_id;
  insert into guardian_account_actions (guardian_id, action, performed_by, reason)
  values (p_guardian_id, 'reactivated', my_employee_id(), null);
end;
$$;
