-- 236: apply_family_checkin の打刻時刻を now()→clock_timestamp()(234のバグ・staging適用済 2026-08-18)。
-- now()はトランザクション固定のため同一tx内の登園/降園が同時刻になり refresh の順序判定が破綻(本番は別tx別時刻で無害だが実時刻が正)。
-- 戻り型は235のまま(out_child_id)。drop→createで再作成。
...
begin
  ...
  for v_sel in select * from jsonb_array_elements(p_selections) loop
    ...
    if v_action = 'checkin' then
      ...
      values (v_child, 'drop_off', clock_timestamp(), v_session.guardian_id);
      ...
      insert into child_attendance_events (child_id, event_type, occurred_at, recorded_by_guardian_id)
      values (v_child, 'pick_up', clock_timestamp(), v_session.guardian_id);
      perform refresh_daily_child_status(v_child, p_business_date);
