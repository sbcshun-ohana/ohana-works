-- 403: 会社負担(自社職員の子)でも備品代は保護者実費として請求する(俊確定 2026-08-31)。
--   Fableレビュー指摘: company_paid児が備品を注文しても run_billing_cycle の
--   company_paidスキップ(continue)で毎月除外され、備品代が永久に請求されなかった。
--   方針: 保育料・給食・延長・調整(a-e)は会社負担で請求しない/承認済み備品注文(f)は請求する。
--   実装: build_child_invoice が company_paid を自動検出し a-e をスキップ(f は常に生成)。
--   → run_billing_cycle も rebuild_child_invoice も呼び出し変更不要で正しく動く。
--   run_billing_cycle は「company_paid かつ 備品注文なし」の時だけ従来どおりスキップ(情報表示)。
--   401の両関数を verbatim 踏襲し、上記の差分のみ追加。

-- ============================================================
-- (1) build_child_invoice — company_paid検出で a-e を自動スキップ(f=備品は常に生成)
-- ============================================================
...
declare
  ...
  v_total int;
  v_item uuid;
  v_company_paid boolean;   -- 403: 会社負担(職員の子)=保育料/給食/延長/調整は請求しない
  r record;
begin
  ...
  end if;  -- not v_company_paid

  -- f) 承認済みの備品注文(未取込分・401)。会社負担でも保護者実費として請求(403)。
  --    1注文=1明細・単価は承認時スナップショット。
  for r in
...
-- ============================================================
-- (2) run_billing_cycle — company_paid かつ 備品注文なし の時だけスキップ(情報表示)。
--     備品注文がある company_paid児は build_child_invoice へ回し備品のみ請求(401をverbatim踏襲)。
-- ============================================================
...
begin
  ...
  loop
    -- 会社負担(職員の子): 保育料・給食・延長は請求しない(build_child_invoiceがa-eを自動スキップ)。
    -- ただし承認済みの備品注文がある場合は保護者実費として請求するため build へ回す(403・俊確定)。
    -- 備品注文が無ければ従来どおり請求書を作らずスキップ(情報表示)。
    if exists (select 1 from child_exemptions x
       ...
       and not exists (select 1 from supply_orders o
                       where o.child_id = v_child.id and o.office_id = p_office_id
                         and o.status = 'approved' and o.invoice_item_id is null) then
