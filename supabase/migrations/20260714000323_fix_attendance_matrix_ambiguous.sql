-- 323: バグ修正(319/320と同種)。fetch_attendance_matrix_for_office(314/318)の実行時例外
--   `column reference "child_id" is ambiguous (42702)` を解消する。
-- 原因: RETURNS TABLE の OUT列 child_id と、days CTE の `select child_id, bd from ev/ab`(bare 参照)が衝突。
--   → 登降園管理ページで在籍児が表示されない不具合。
-- 修正: 関数先頭に #variable_conflict use_column(曖昧参照は列優先)。最終SELECTは位置対応で返すため出力不変。
-- signature 不変=create or replace・ロジックは318と同一。
